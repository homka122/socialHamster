...
class MessagesController {
  ...
  sendMessageToUser = catchAsync(async (req, res, next) => {
    ...
    const conversationCandidate = await ConversationRepository.findOne({ $or: [{ user1: req.user._id, user2: receiver._id }, { user1: receiver._id, user2: req.user._id }] })
    ...
    // server.send(JSON.stringify({ event: 'sendMessage', sender: req.user, receiver: receiver, message, password: process.env.WS_PASSWORD }))
    // server.close();
    //
