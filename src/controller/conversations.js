...
class ConverationsController {
  ...
  getUserConversation = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    ...
    if (!userConversation.user1.equals(req.user._id) && !userConversation.user2.equals(req.user._id)) {
      return next(new ApiError('Нет доступа', 400));
    }

    const formatUserConversation = (conversation) => {
      let conversationForSend = { ...conversation.toObject() };
